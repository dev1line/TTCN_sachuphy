						...
						<div class="row gutters-tiny" style="justify-content: center">
							...
							<div class="col-6 px-2">
								...
								<b-form-group label="Feature" label-for="block-form5-username">
									<b-button
										v-for="(feature, index) in tempFeatures"
										:key="feature.index"
										@click="
											showEditFeature('modal-edit-feature', feature, index)
										"
									>
								...
								<b-form-group label="Storage" label-for="block-form5-username">
									<b-button
										v-for="(storage, index) in tempStorage"
										@click="
											showEditStorage('modal-edit-storage', storage, index)
										"
										:key="storage.index"
									>
							...
							<div class="col-auto px-2">
								<b-button
									variant="success"
									v-if="isEditing"
									@click="editOptionConfirm"
								>
									Sửa
								</b-button>
								<b-button v-else variant="success" @click="createOptionConfirm"
									>Thêm</b-button
								>
							</div>
					...
					<div class="block-content block-content-full">
						<div class="row gutters-tiny" style="justify-content: center">
							...
							<div class="col-auto px-2">
								<b-button
									variant="light"
									@click="hideModal('modal-edit-options')"
									>Hủy</b-button
								>
							</div>
						</div>
...
export default {
	...
	methods: {
		editOptionConfirm() {
			const productPayload = this.formOption
			const slug = productPayload.slug
			const index = this.currentOptions.findIndex(
				(o) => o.slug === productPayload.slug
			)
      this.updateProduct({ index, slug, type: "options", productPayload })
      this.$bvModal.hide('modal-create-option')
		...
		showEditOption(optionItem) {
			...
			this.tempStorage = optionItem.storage
			this.tempMemory = optionItem.memory
			this.tempFeatures = optionItem.features
			this.tempDisplay = optionItem.display
		},
		createOptionConfirm() {
